...
type Screen = 'loading' | 'landing' | 'login' | 'preferences' | 'home' | 'preview';
...
export default function App(): React.JSX.Element {
  ...
  const [activeScanMode, setActiveScanMode] = useState<ScanModeId>('anything');

  const loadUser = async (uid: string) => {
    const saved = await preferencesService.get(uid);
    if (!saved) { setScreen('preferences'); return; }
  ...
  useEffect(() => {
    ...
    if (!current) { setScreen('landing'); return; }
  ...
  const signInWithGoogle = async () => {
    ...
    try { const user = await authService.signInWithGoogle(); await loadUser(user.uid); }
  ...
  const finishPreferences = async (value: PreferenceSetupValue) => {
    ...
    setLanguageTag(value.defaultListeningLanguage); setDefaultMode(value.defaultScanMode); setScreen('home');
  };

  const startScan = async (mode: ScanModeId) => {
    if (captureBusy) return;
    setCaptureBusy(true); setCaptureError(null); setActiveScanMode(mode);
    try {
      const photo = await capturePhoto();
      if (photo) { setCapturedPhoto(photo); setScreen('preview'); }
    } catch (error) {
